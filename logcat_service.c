...
// FIXME add abstraction layer for uv_poll primitive
...
static void logcat_on_data_available(uv_poll_t* handle, int status, int events);
...
static void logcat_on_data_available(uv_poll_t* handle, int status, int events) {
    ...
    ret = read(handle->io_watcher.fd, ap->p.data, CONFIG_ADB_PAYLOAD_SIZE);
    ...
    adb_client_send_service_payload(&client->client, &ap->p);
    return;
...
adb_service_t* logcat_service(adb_client_t *client, const char *params) {
    ...
    // TODO parse params
    char *ptr = strstr(params, "exec logcat");
    adb_log("LOGCAT PARAMS <%s>\n", ptr+5);

    ret = open(CONFIG_SYSLOG_DEVPATH, O_RDONLY);
